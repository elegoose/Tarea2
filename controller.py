...
def camerapos(bodies, controller):
    ...
    for body in bodies:
        if controller.bodyID == 0:
            ...
            # theta = body.theta + controller.movea
            # cameradistance = body.distance + controller.mover
            camtheta = - 0.65
            ...
            for satellite in body.satellites:
                if controller.bodyID == satellite.bodyID:
                    # theta = satellite.theta + controller.movea
                    # cameradistance = satellite.distance + controller.mover
                    camtheta =  - 0.76
                    cameradistance = satellite.distance + 0.01
                    eyeposx = cameradistance * np.cos(satellite.theta + camtheta) + body.posx
                    eyeposy = cameradistance * np.sin(satellite.theta + camtheta)/np.sqrt(1+satellite.inclination ** 2) + body.posy
                    eyeposz = cameradistance * satellite.inclination * np.sin(satellite.theta + camtheta)/np.sqrt(1+satellite.inclination ** 2) + body.posz
                    eye = np.array([eyeposx, eyeposy, eyeposz+0.05])
                    viewPos = np.array([satellite.posx+body.posx, satellite.posy+body.posy, satellite.posz+body.posz])
                    # viewPos = np.array([eyeposx, eyeposy, eyeposz])
                    # return eye, viewPos
                    return viewPos, eye
    return viewPos, eye
